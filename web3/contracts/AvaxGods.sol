...
contract AVAXGods is ERC1155, Ownable, ERC1155Supply {
    ...
    struct GameToken {
        ...
        uint256 id; ///@param id battle card token_id; generated randomly
    ...
    struct Player {
        ...
        uint256 playerMana; ///@param playerMana player mana; battle stats
        uint256 playerHealth; ///@param playerHealth player health; battle stats
        bool inBattle; ///@param inBattle boolean to indicate if player is in battle
    }
    ...
    function getPlayer(address addr) public view returns (Player memory) {
        require(isPlayer(addr), "Player does not exist!");
    ...
    function isPlayerToken(address addr) public view returns (bool) {
     ...
     function getPlayerToken(address addr) public view returns (GameToken memory) {
        require(isPlayerToken(addr), "Player does not exist!");
    ...
    function registerPlayer(string memory _name, string memory _gameTokenName) external {
        require(!isPlayer(msg.sender), "Player already registered"); //check that player is not alr registered

        uint256 _id = players.length;
        players.push(Player(msg.sender), _name, 10, 25, false); //Adds new Player to players array
        playerInfo[msg.sender] = _id; //Updates player info mapping
    ...
    function createRandomGameToken(string memory _name) public {
        require(!getPlayer(msg.sender).inBattle, "Player is in a battle"); //check that player is not in battle
        require(isPlayer(msg.sender), "Please register player first"); //check that player is alr registered
    ...
    function createBattle(string memory _name) external returns (Battle memory) {
        require(isPlayer(msg.sender), "Please register player first"); //player must be registered in order to create battle 
        require(!isBattle(_name), "Battle already exists!"); //cannot have duplicate battle name
        ...
        Battle memory _battle = Battle(
            BattleStatus.PENDING, // set battle status to PENDING
            battleHash, // battle hash
            _name, //battle name
            [msg.sender, address(0)], //setting player address; player 2 empty until someone joins battle
            [0,0], //moves for each player
            address(0) //winner address set to empty until battle ends
        );

        //add newly initialized battle into array of exisiting battles
        uint256 _id = battle.length;
    ...
    function joinBattle(string memory _name) external returns (Battle memory) {
        //retrieving battle from array of battles
        Battle memory _battle = getBattle(_name);

        //checks
        require(_battle.battleStatus == BattleStatus.PENDING, "Battle already started!"); //can only join battles that has not started
        require(_battle.players[0] != msg.sender, "Only player 2 can join a battle!"); //need another player to join the player other than creator of battle
        require(!getPlayer(msg.sender).inBattle, "Already in a battle!"); //require that player is not already in a battle

        //updates battle status and players array
        _battle.battleStatus = BattleStatus.STARTED;
        _battle.players[1] = msg.sender;
        updateBattle(_name, _battle);

        //updates players battle status
        players[playerInfo[_battle.players[0]]].inBattle = true;
        players[playerInfo[_battle.player[1]]].inBattle = true;

        emit NewBattle(_battleName, _battle.players[0], msg.sender); //emits battle has begun
        return _battle;
    }

    //fetch battle moves for player1 and player2
    ...
    function _resolveBattle(Battle memory _battle) internal {
        P memory p1 = P(
            ...
            getPlayerToken(_battle.players[0]).playerStrength,
        ...
        P memory p2 = P(
            ...
            _battle.moves[1],
            getPlayer(_battle.player[1]).playerHealth,
            getPlayerToken(_battle.players[1]).playerStrength,
             getPlayerToken(_battle.players[0]).defenseStrength
        );
    ...
    function _endBattle(address battleEnder, Battle memory _battle) internal returns (Battle memory) {
        ...
        emit BattleEnded(_battle.name, battleEnder, _battleLoser); //Emit BattleEnded event
